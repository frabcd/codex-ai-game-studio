...
_CACHE_ROOT_ENV: Final = "AI_GAME_STUDIO_CACHE_DIR"
...
def _profile_cache_path(mapping: dict[str, JsonValue], path: Path) -> str:
    ...
    if cache_path.is_absolute() or ".." in cache_path.parts:
        raise ProfileCachePathError(
            "cache path must be relative and stay within cache_root"
        )
    return configured
...
def default_cache_root() -> Path:
    """Return a per-user writable cache root outside the installed skill."""
    explicit = os.environ.get(_CACHE_ROOT_ENV)
    if explicit:
    ...
    return root / "codex-ai-game-studio" / "img2threejs"


def _is_bundled_skill_root(source_root: Path) -> bool:
    plugin_root = source_root.parent.parent
    return (
        source_root.parent.name == "skills"
...
def _cache_path(request: IndexRequest) -> Path:
    ...
    try:
        source_root = request.project_root.expanduser().resolve()
        if request.cache_root is not None:
            cache_root = request.cache_root.expanduser().resolve()
        elif _is_bundled_skill_root(source_root) or os.environ.get(_CACHE_ROOT_ENV):
            cache_root = default_cache_root().resolve()
        else:
            # A standalone user-owned skill checkout remains a valid project
            # root. Bundled plugin sources take the per-user path above.
            cache_root = source_root
        cache_path = (cache_root / configured).resolve()
    ...
    if cache_path == cache_root:
        raise _cache_validation_error(
            configured,
            "cache path must not replace cache_root",
        )
    if _is_bundled_skill_root(source_root):
        try:
            cache_path.relative_to(source_root)
        except ValueError:
            pass
        else:
            raise _cache_validation_error(
                cache_path,
                "cache_root must stay outside the installed img2threejs source tree",
            )
    return cache_path
