...
def search_local_specs(
    ...
    loaded = load_or_build_index(
        IndexRequest(PROJECT_ROOT, collection, profile, force_reindex, cache_root)
    )
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Approved writable spec-search cache root. Defaults to "
            "AI_GAME_STUDIO_CACHE_DIR or the operating system user cache; "
            "must be outside the installed skill."
        ),
    )
