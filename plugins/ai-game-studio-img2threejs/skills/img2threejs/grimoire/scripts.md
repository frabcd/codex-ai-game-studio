...
Playwright/Chromium. PNG read/write is done via `struct`/`zlib`. Resolve the directory containing
`SKILL.md`, then invoke each script with the active interpreter and its absolute
`<skill-root>/forge/<name>.py` path. This works with `py -3` or `python` on Windows and `python3`
or the configured environment on macOS. Non-zero exit = a gate failed; read the printed reasons.
