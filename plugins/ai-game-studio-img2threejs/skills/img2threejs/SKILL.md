...
[UPSTREAM.json](../../UPSTREAM.json) and [LICENSE](../../LICENSE) before redistributing changes.

## Core Promise

Sculpt from a photo, in order — never one-shot a mesh:
1. **Run `<python> "<skill-root>/forge/next.py" <spec>` first.** It reports the current unlocked pass, exact next command, and unmet acceptance criteria.
