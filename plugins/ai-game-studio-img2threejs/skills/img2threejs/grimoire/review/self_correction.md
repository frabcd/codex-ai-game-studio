...
The proper fix is a Divine-Eye **reconstruction mode** (reference==photo ⇒ drop pixel-aligned
signals, align-then-IoU, palette+objectness). The bundled
`forge/stage4_review/objectness.py` implementation provides the OSIM-lite signal used by that mode;
inspect its emitted evidence instead of relying on host-local memory or an unbundled review log.
